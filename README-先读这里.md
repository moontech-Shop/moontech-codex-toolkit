...
依赖：Windows PowerShell **7**（不是5.1）；Codex任务管理工具用于自动创建持久任务。缺少任务工具时只生成目录与TASK-STARTERS，不声称创建了任务。浏览器、Shopify、Git、图片工具由运行环境提供；缺少时继续可做的本地工作并说明限制。
...
当前补丁包含Logo设计确认和严格复刻素材迁移规则；手册路径沿用v1.2。历史ZIP是旧版快照，安装最新版请使用v1.2.2。
